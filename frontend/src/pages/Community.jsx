  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[0.8fr_1.2fr]">
      <section className="panel h-fit">
        ...
        <button className="btn-primary mt-4 w-full" onClick={publish}><Send size={16} /> Publish</button>
      </section>
        ...
        {posts.map((post) => (
          <article key={post._id} className="panel">
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              <button className="btn-secondary px-4 py-2" onClick={() => like(post._id)}><Heart size={16} /> {post.likes?.length || 0}</button>
              <button className="btn-secondary px-4 py-2"><MessageCircle size={16} /> {post.comments?.length || 0}</button>
              <button className="btn-secondary px-4 py-2" onClick={() => report(post._id)}><Flag size={16} /> Report</button>
            </div>
            <div className="mt-4 grid gap-3">
              ...
              <div className="flex gap-2">
                ...
                <button className="btn-primary px-4 py-2" onClick={() => comment(post._id)}><Send size={16} /></button>
              </div>
            </div>
          </article>
        ))}
        {posts.length === 0 && <div className="panel text-sm text-slate-600 dark:text-slate-300">No community posts yet. The first supportive note can be small.</div>}
      </section>
    </div>
  );
}
